...
cli 示例覆盖四种功能（`role.cli.function` 取值 `upload` / `file` / `ejson` / `sql`；另有
`config` / `configget` 配置发布/读取功能，无独立示例配置），各一套 max/min × yaml/json，
共 16 份（`cli.upload.*` / `cli.file.*` / `cli.ejson.*` / `cli.sql.*`）；upload / ejson / sql 以管道喂 stdin 运行，
file 不读 stdin（输入来自 `source.file.paths` 匹配的存量文件），均无 `start.sh`。
...
| cli (file) | `cli`  | [cli/](cli/)               | logging · dao · parser · process · source.file · role.cli（`function=file`，存量文件一次性导入） |
...
# cli file 不读 stdin（输入来自 source.file.paths 匹配的存量文件，读到 EOF 即止，统计 JSON 写 stdout）：
tango --config examples/config/cli/cli.file.min.yaml
...
| cli (file) | `dao.mongo.uri`、`role.cli.function: file`、`source.file.paths` |
