...
// WHERE clause body (without the leading "WHERE"). The returned SQL string has
// the shape:
//
...
//
//   - Identifiers (including hash-prefixed via the same rewrite used by the
...
// hash-rewrite pattern, string methods, regex, etc.) cause a compile error
// identifying the offending source fragment. The caller is expected to either
// rewrite the expression to a pushdown-safe form or accept that the local
// filter alone will enforce it.
...
// and emits a quoted column reference. Anything else (property access on
// arbitrary objects, optional chaining, slice/index expressions) is rejected.
