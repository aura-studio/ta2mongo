...
//
// PageID/PageCount/Rows deliberately omit `omitempty`: PageID's initial value
// is -1 and its post-first-page value is 0, both of which omitempty would
// mangle (it only omits the zero value), so the stored state would be
// ambiguous on resume. Storing them unconditionally keeps resume exact.
type DayProgress struct {
	Status    DayStatus `bson:"status"`
	TaskID    string    `bson:"taskId,omitempty"`
	PageID    int       `bson:"pageId"`    // last fully processed page (-1 = none yet)
	PageCount int       `bson:"pageCount"` // populated once the task is FINISHED
	Rows      int64     `bson:"rows"`      // rows ingested so far for this day
	Error     string    `bson:"error,omitempty"`
}

// Run is one backfill execution's checkpoint document (collection
// _backfill_progress, _id == RunID).
type Run struct {
	ID           string                 `bson:"_id"`
...
// because the document is loaded into memory and replaced wholesale (the
// ReplaceOne is last-writer-wins, so one RunID must have one writer — the
// single-replica deployment model satisfies this).
...
// match the one computed from the current configuration. The caller should fix
// the config drift or pick a new RunID.
...
// looks like YYYY-MM-DD. When start equals UserChunkKey, the run is treated as
// having a single chunk (no time expansion) — used for user-table sync.
func NewCheckpoint(ctx context.Context, coll *mongo.Collection, runID, apiBaseURL string,
	...
	case err == nil:
		...
		cp.run = &existing
		// Initialise any new days that fall in the range but weren't present
		// before (e.g. the user extended the range).
...
// order. Days marked Completed are skipped; InProgress, Pending and Failed days
// are all returned (the runner resumes in-flight days first).
...
// ReplaceOne is the DocumentDB-safe write form here — never rewrite it as a
// pipeline-form update, which DocumentDB rejects.
...
// SQLSignature computes a deterministic short hash over the SQL template pieces
// that, if changed, invalidate prior progress (table, project, filter WHERE,
// event-time bounds). The partition-date range is deliberately excluded so a
// run may be extended to more days and resumed.
...
// initChunks expands the (start, end) pair into a map of Pending entries. For a
// YYYY-MM-DD date range, one entry per day. UserChunkKey collapses to a single
// entry (user-table sync has no time partition and runs as one SQL task).
