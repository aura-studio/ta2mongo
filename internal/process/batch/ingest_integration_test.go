...
func TestNewFromClient(t *testing.T) {
	...
	line := `{"#type":"track","#event_name":"test","#time":"2024-01-01","#uuid":"fc-1","#account_id":"fc_acc"}`
	if err := ig.Ingest(ctx, line); err != nil {
		t.Fatalf("Ingest via NewFromClient: %v", err)
	}

	count, _ := client.Database(dbName).Collection("event").CountDocuments(ctx, bson.M{})
	if count != 1 {
