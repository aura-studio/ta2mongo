// Package core holds the shared per-line ingestion semantics — parse, filter,
// identity resolve, and write-model construction — plus the stats types, used by
// all three upload strategies (single / batch / pipeline), so the rules (what
// becomes a user/event write, what is filtered, what goes to dead letter) live
