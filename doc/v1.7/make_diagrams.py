#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate tango v1.7 architecture diagrams (PIL only, CJK fonts).

v1.7 = v1.6（file 一次性文件导入）+ backfill（TA OpenAPI 历史回填，新域 internal/backfill）。
沿用 doc/v1.5 / doc/v1.6 的画法（分层彩色圆角盒 + 左→右流程图），2x 超采样后 LANCZOS 缩放。
相对 v1.6：overview 增 backfill 顶层域 + RunBackfill 入口；新增一张 backfill-flow 图（图 D）。
输出：doc/v1.7/{overview,upload-flow,cfgsync-flow,file-flow,backfill-flow}.png
"""
import os
...
C_BF = ("#e4dcf3", "#7e6bbf")    # v1.7 backfill 高亮（紫）
INK = "#222222"
SUBINK = "#555555"
LINE = "#9aa3ad"
...
# ===========================================================================
# Diagram 1: overview  (v1.7 = v1.6 + backfill 顶层域)
# ===========================================================================
def overview():
    W, H = 1400, 860
    img, d = new_canvas(W, H)
    ctext(d, W / 2, 22, "tango v1.7 架构总览 — 单一二进制 · 上报日志 + Data API + cfgsync + file 导入 + backfill 历史回填", 19, INK, bold=True)
    ctext(d, W / 2, 50, "运行角色由配置键 role.mode 选定（daemon / gateway / cli）；api 是被内嵌的引擎库，不可派发", 12, SUBINK)
    ...
    roles = [
        ...
        ("cli", ["stdin / 文件 / 回填 一次性", "upload·file·backfill·ejson·sql·config·configget"]),
    ]
    for i, (t, subs) in enumerate(roles):
        box(d, GX0 + i * (rw + 18), 156, rw, 64, C_ROLE, title=t, subs=subs, tpx=15, spx=8.6)
    ...
    box(d, GX0, 230, GW, 32, ("#eef3fa", "#7aa0c4"),
        subs=["api.Engine（被 gateway / cli / client 内嵌）：New / NewFromTree · Upload / File / RunBackfill / Run · EJSON · SQL · EnsureIndexes · StartCfgsync · PublishConfig · Close"],
        spx=10.5)

    # 编排领域 / 有界任务
    rowlabel(300, "编排领域", "/ 有界任务")
    box(d, GX0, 288, GW, 72, C_ORCH,
    ...
    for i, lines in enumerate(subs_rows):
        ...
        box(d, GX0 + i * (cw + 18), 506, cw, 58, col, subs=lines, spx=10.5)

    for i in range(4):
        cx = GX0 + i * (cw + 18) + cw / 2
        arrow(d, cx, 486, cx, 504, color=LINE, width=1.6, head=7)
    ...
    for i, (t, sub) in enumerate(bases):
        box(d, GX0 + i * (bw + 18), 582, bw, 46, C_BASE, title=t, subs=[sub], tpx=14, spx=9.5)
    ...
    # legend
    ly = 704
    box(d, GX0, ly, GW, 134, ("#fafafa", "#cccccc"), radius=8)
    ltext(d, GX0 + 18, ly + 12, "图例", 12, INK, bold=True)
    keys = [("角色层 role", C_ROLE), ("编排 cfgsync", C_ORCH), ("backfill 域(v1.7)", C_BF),
            ("引擎根包", C_ROOT), ("子包", C_SUB), ("file 源(v1.6)", C_FILE), ("基础", C_BASE)]
    kx = GX0 + 90
    ...
    ltext(d, GX0 + 18, ly + 114,
          "· 相对 v1.4：移除 worker / taskqueue / fileupload / filebatch；remoteconfig 收敛为 cfgsync；SQL 改为注入式依赖外部 mongosql。", 10.5, "#a05a1a")

    finish(img, os.path.join(HERE, "overview.png"))


# ===========================================================================
# Diagram 2: upload-flow  (与 v1.6 同构；仅版本号)
# ===========================================================================
def upload_flow():
    W, H = 1400, 620
    img, d = new_canvas(W, H)
    ctext(d, W / 2, 22, "图 A · 单行上报数据流（v1.7）", 20, INK, bold=True)
    ...
    ltext(d, 720, 522, "file 一次性导入（图 C）与 backfill event 路（图 D）都复用这套同核管线", 11, "#1b6f5c")

    finish(img, os.path.join(HERE, "upload-flow.png"))


# ===========================================================================
# Diagram 3: cfgsync-flow  (与 v1.6 一致，仅版本号)
# ===========================================================================
def cfgsync_flow():
    W, H = 1400, 640
    img, d = new_canvas(W, H)
    ctext(d, W / 2, 22, "图 B · cfgsync 读写同核（v1.7）", 20, INK, bold=True)
...
# ===========================================================================
# Diagram 4: file-flow  (与 v1.6 一致，仅版本号)
# ===========================================================================
def file_flow():
    W, H = 1400, 640
    img, d = new_canvas(W, H)
    ctext(d, W / 2, 22, "图 C · file 存量文件一次性导入（v1.7；v1.6 落地）", 20, INK, bold=True)
